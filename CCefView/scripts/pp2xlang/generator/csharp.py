...
class CSharpGenerator(SourceGenerator):
    ...
    def marshal_params(self, params, args, out_params, out_args):
        for i in range(0, len(params)):
            ...
            if p.basic.startswith("CCef") and p.basic in self.types.keys():
            ...
            elif p.basic.startswith("bool"):
                out_params.append(f"[MarshalAs(UnmanagedType.Bool)] " + p.spelling)
                out_args.append(a)
                ...
                out_params.append(
                    f"[MarshalAs(UnmanagedType.LPArray, SizeParamIndex = {len(out_params) + 1})] "
                    + p.spelling
                )
                out_args.append(a)
            else:
    ...
    def marshal_return(self, type):
        print(type)
        if type.startswith("void"):
            return f"// No Return Value"
        elif type.startswith("bool"):
            return f"[return: MarshalAs(UnmanagedType.Bool)]"
        else:
            return ""
    ...
    def translate_class(self, cursor: clang.cindex.Cursor):
        ...
        class_header = (
            ...
            f"        internal static extern void {cpp_class}_Delete(IntPtr p);\n"
            ...
            match c_cursor.kind:
                case clang.cindex.CursorKind.CONSTRUCTOR:
                    ...
                    constructor = (
                        ...
                        f'        [DllImport("{self.dll}")]\n'
                        f'        internal static extern IntPtr {cpp_class}_new{constructor_index}({", ".join(marshal_params)});\n'
                        f"\n"
                    )
                ...
                case clang.cindex.CursorKind.CXX_METHOD:
                    ...
                    marshal_return = self.marshal_return(cpp_return_type.spelling)
                    ...
                    method = (
                        ...
                        f'        [DllImport("{self.dll}")]\n'
                        f"        {marshal_return}\n"
                        f'        internal static extern {self.translate_type(cpp_return_type, True)} {cpp_class}_{cpp_method_name}({", ".join(marshal_params)});\n'
