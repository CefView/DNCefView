...
if __name__ == "__main__":
    ...
    parser.add_argument("--cef-include-path", required=True)
    parser.add_argument("--c-gen-out-dir", required=True)
    parser.add_argument("--cs-gen-out-dir", required=True)
    parsed = parser.parse_args()

    # resolve to absolute paths
    cef_include_path = os.path.abspath(parsed.cef_include_path)
    c_gen_out_dir = os.path.abspath(parsed.c_gen_out_dir)
    cs_gen_out_dir = os.path.abspath(parsed.cs_gen_out_dir)

    # create output directories if not exist
    os.makedirs(c_gen_out_dir, exist_ok=True)
    os.makedirs(cs_gen_out_dir, exist_ok=True)
    ...
    cgen = CGenerator(out=c_gen_out_dir, mapper=CTypeMapper())
    translator.add_generator(cgen)

    sharpgen = CSharpGenerator(
        out=cs_gen_out_dir,
        namespace="DNCefView",
        dll="CCefView",
        mapper=CSharpTypeMapper(),
    )
    translator.add_generator(sharpgen)
