                                           ...
                                           const std::vector<CefDraggableRegion>& regions)
{
  ...
  if (pCefView_->callbackTable_.pfnDraggableRegionChanged)
    pCefView_->callbackTable_.pfnDraggableRegionChanged(regions.data(), regions.size());
}
...
CCefClientDelegate::addressChanged(CefRefPtr<CefBrowser>& browser, int64_t frameId, const std::string& url)
{
  ...
  if (pCefView_->callbackTable_.pfnAddressChanged)
    pCefView_->callbackTable_.pfnAddressChanged(frameId, url.c_str());
}
...
CCefClientDelegate::titleChanged(CefRefPtr<CefBrowser>& browser, const std::string& title)
{
  ...
  if (pCefView_->callbackTable_.pfnTitleChanged)
    pCefView_->callbackTable_.pfnTitleChanged(title.c_str());
}
...
CCefClientDelegate::faviconChanged(CefRefPtr<CefImage> image)
{
  ...
  // pCefView_->q_ptr->faviconChanged(icon);
}
...
CCefClientDelegate::statusMessage(CefRefPtr<CefBrowser>& browser, const std::string& value)
{
  ...
  if (pCefView_->callbackTable_.pfnStatusMessage)
    pCefView_->callbackTable_.pfnStatusMessage(value.c_str());
}
...
CCefClientDelegate::consoleMessage(CefRefPtr<CefBrowser>& browser, const std::string& message, int level)
{
  ...
  if (pCefView_->callbackTable_.pfnConsoleMessage)
    pCefView_->callbackTable_.pfnConsoleMessage(message.c_str(), level);
}
