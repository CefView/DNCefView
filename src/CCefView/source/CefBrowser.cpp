...
namespace {
inline CefRefPtr<CefBrowserHost> GetWindowlessHost(const CefRefPtr<CefBrowser>& browser)
{
  if (!browser)
    return nullptr;
...
inline CefMouseEvent BuildMouseEvent(int x, int y, uint32_t modifiers)
{
  CefMouseEvent event;
  event.x = x;
  event.y = y;
...
std::vector<std::string> SplitFilePaths(const std::string& filePaths)
{
  std::vector<std::string> paths;
  std::stringstream stream(filePaths);
  std::string line;
...
CCefBrowser::start()
{
  ...
  if (windowlessRenderingEnabled) {
    ...
    window_info.shared_texture_enabled = hardwareAccelerationEnabled;
    window_info.external_begin_frame_enabled = externalBeginFrameEnabled;
  }
...
{
  if (pClientDelegate_) std::static_pointer_cast<CCefClientDelegate>(pClientDelegate_)->detach();
  resetSourceDragState();
  clearJSDialogCallbacks();
  // ABI 3 maps: dropping the CefRefPtr here cancels any CEF callback still alive,
  // so a destroyed browser cannot leak one or fire into a dead host.
  clearFileDialogCallbacks();
  clearBeforeDownloadCallbacks();
  clearDownloadItemCallbacks();
  clearContextMenuCallbacks();
  clearPermissionPromptCallbacks();
...
{
  if (forceClose && pClient_) {
    // OnAfterCreated runs before CefViewCore inserts into its browser map.
    // Defer so a close requested during creation sees that browser too.
    CefPostTask(TID_UI, base::BindOnce([](CefRefPtr<CefViewBrowserClient> client) { client->CloseAllBrowsers(); }, pClient_));
    return;
  }
  if (!pCefBrowser_)
    return;
...
{
  auto host = GetWindowlessHost(pCefBrowser_);
  if (!host)
    return;
  ...
  host->DragTargetDragEnter(dragData, BuildMouseEvent(x, y, modifiers),
                               ...
                               CefViewRange selection_range)
{
  ...
  if (count < 0 || (count > 0 && !underlines)) return;
