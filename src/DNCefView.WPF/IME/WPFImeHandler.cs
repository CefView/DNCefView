    ...
    public class WPFImeHandler
    {
        ...
        private void OnIMEComposition(IntPtr hwnd, int lParam)
        {
            ...
            using (var handler = IMEHandler.Attach(hwnd))
            {
                ...
                {
                    _owner.ImeCommitText(text, new CefViewRange(0xFFFFFFFF, 0xFFFFFFFF), 0);
                    return;
                }
            }
            ...
            using (var handler = IMEHandler.Attach(hwnd))
            {
                ...
                uint compositionStart = 0;

                if (handler.GetComposition((uint)lParam, underlines, ref compositionStart, out text))
                {
                    _owner.ImeSetComposition(text, underlines.ToArray(), new CefViewRange(0xFFFFFFFF, 0xFFFFFFFF), new CefViewRange(compositionStart, compositionStart));
                }
            }
        }
