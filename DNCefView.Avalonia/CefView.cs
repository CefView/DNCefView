...
{
    private CefSetting? _cefSetting;
    public CefSetting? Setting => _cefSetting;
    ...
    public delegate bool LoadErrorCallback(int browserId, string frameId, bool isMainFrame, int errorCode,
        string errorMsg, string failedUrl);
    ...
    public void ImeSetComposition(string text, CefViewCompositionUnderline[] underlines, CefViewRange replacementRange,
        CefViewRange selectionRange)
    {
        _cefBrowser?.ImeSetComposition(text, underlines, underlines.Length, replacementRange, selectionRange);
    }

    public void ImeCommitText(string text, CefViewRange replacementRange, int relativeCursorPos)
    {
        _cefBrowser?.ImeCommitText(text, replacementRange, relativeCursorPos);
    }

    public void ImeFinishComposingText(bool keepSelection)
    {
        _cefBrowser?.ImeFinishComposingText(keepSelection);
    }
    ...
    void ICefViewDelegate.OnCefLoadStart(int browserId, string frameId, bool isMainFrame, int transitionType)
    {
        LoadStart?.Invoke(browserId, frameId, isMainFrame, transitionType);
    }
    ...
    bool ICefViewDelegate.OnCefLoadError(int browserId, string frameId, bool isMainFrame, int errorCode,
        string errorMsg, string failedUrl)
    {
        if (null != LoadError)
    ...
    void ICefViewDelegate.OnCefPaint(int browserId, CefViewPaintElementType type, CefViewRect[] dirtyRects,
        int dirtyRectCount, IntPtr imageBytesBuffer, int imageBytesCount, int width, int height)
    {
        UI_OnCefPaint(browserId, type, dirtyRects, dirtyRectCount, imageBytesBuffer, imageBytesCount, width, height);
    }

    void ICefViewDelegate.OnCefAcceleratedPaint(int browserId, CefViewPaintElementType type, CefViewRect[] dirtyRects,
    ...
    void ICefViewDelegate.OnCefImeCompositionRangeChanged(int browserId, CefViewRange selectedRange,
        CefViewRect[] characterBounds, int characterBoundsCount)
    {
